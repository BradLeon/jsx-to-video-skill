---
name: jsx-to-gif-all
description: Convert JSX/React animation into ONE combined GIF file containing all steps playing in sequence. Use when the user wants a single GIF with all animation steps. Trigger keywords: 'JSX转GIF', '完整GIF', '合并GIF', 'combined GIF', 'one GIF', '整体GIF', '全部步骤一个GIF', '动画转GIF'. Also trigger when user says 'make one GIF of the whole thing' or '合成一个GIF'.
---

# jsx-to-gif-all

**Output: One single GIF file** containing all steps playing in sequence with auto-advance.
...
# Encode ONE GIF from all frames
...
```bash
xvfb-run -a python ${CLAUDE_PLUGIN_ROOT}/scripts/capture.py --html animation.html --output ./output --combined --gif-only
```
