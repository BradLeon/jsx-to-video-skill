---
name: jsx-to-mp4-separate
description: Convert JSX/React animation into SEPARATE MP4 video files — one MP4 per step/page. Use when the user wants individual video clips for each step in a multi-step JSX component. Trigger keywords: 'JSX转MP4', '分页MP4', 'separate MP4', '每一页MP4', '逐步导出视频', 'MP4 per step', '动画转视频分页'. Also trigger when user says 'export each step as video' or '每步一个视频'.
---

# jsx-to-mp4-separate

**Output: One MP4 file per step.** If the JSX has N steps, produce N MP4 files named `01_步骤名.mp4`, `02_步骤名.mp4`, etc.
...
```bash
xvfb-run -a python ${CLAUDE_PLUGIN_ROOT}/scripts/capture.py --html animation.html --output ./output --mp4-only
```
